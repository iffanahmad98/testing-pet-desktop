...
public class HotelFacilitiesMenu : HotelShopMenuBase
{
    ...
    public Button GetFirstHireButton()
    {

        if (listPodiumCard == null || listPodiumCard.Count == 0)
            return null;

        var card = listPodiumCard[0];
        return card != null ? card.hireButton : null;
    }

    public Button GetFirstApplyButton()
    {
        ...
        var card = listPodiumCard[0];
        return card != null ? card.applyButton : null;
    }
    ...
    {
        if (dictionaryHiredMaxFacility.ContainsKey(data.id))
        { // data.maxHired > 0
